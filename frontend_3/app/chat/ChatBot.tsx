...
export function ChatBot({ selectedModel, activeChatId, onChatCreated, onPanelStateChange }: ChatBotProps) {
  ...
  // Draft panel state (keep DraftPanel unchanged)
  ...
  const handleDraftContentChange = (draftId: string, content: string) => {
    setDrafts((prev) =>
      prev.map((d) => (d.id === draftId ? { ...d, content, updatedAt: new Date() } : d))
    )
  }
  ...
  const handleRenameDraft = (draftId: string, newTitle: string) => {
    setDrafts((prev) =>
      prev.map((d) => (d.id === draftId ? { ...d, title: newTitle, updatedAt: new Date() } : d))
    )
  }
