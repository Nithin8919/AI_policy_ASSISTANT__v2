...
export async function aiEditDraft(request: AIEditDraftRequest): Promise<AIEditDraftResponse> {
  try {
    console.log('🔵 AI Edit Draft called with:', { instruction: request.instruction, draftLength: request.draft.length })

    const response = await fetch(`${API_BASE_URL}/api/ai-edit-draft`, {
      method: 'POST',
      headers: {
        'Content-Type': 'application/json',
      },
    ...
    if (!response.ok) {
      const errorText = await response.text()
      throw new Error(`AI edit failed: ${response.status} - ${errorText}`)
    }

    const data = await response.json()
    console.log('✅ AI edit response:', { editedDraftLength: data.editedDraft?.length })
    return data
  } catch (error) {
    console.error('❌ Error calling AI edit draft API:', error)
    throw error
  }
}
